...
class Call(Base):
    ...
    insights: Mapped["Insight"] = relationship(back_populates="call", uselist=False, cascade="all, delete-orphan")
...
class Insight(Base):
    ...
    sentiment_timeline: Mapped[dict] = mapped_column(JSONB, default=list)
    call_score: Mapped[dict] = mapped_column(JSONB, default=dict)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
